...
namespace detail_rand {
  template<class T>
  static auto test_setRThetaPhi(int)->decltype(void(std::declval<T>().setRhoPhiTheta(0.,0.,0.)), std::true_type{});
  template<class T>
  static auto test_setRThetaPhi(float) ->std::false_type;

  template<class T>
  static auto test_setRhoPhiZ(int)->decltype(void(std::declval<T>().setRhoPhiZ(0.,0.,0.)), std::true_type{});
  template<class T>
  static auto test_setRhoPhiZ(float) ->std::false_type;

  template<class T>
  static auto test_setX(int)->decltype(void(std::declval<T>().setX(0.)), std::true_type{});
  template<class T>
...
public:
  ...
  inline static double Uniform(double min, double max) { return min + (max-min) * UniformRand(); }
  ...
  inline static double Gauss(double mean, double stddev) { return mean + stddev * GaussianRand(); }
  ...
  inline static bool Bernouilli(double p) { SetBernouilliPar(p); return BernouilliRand(); }
  ...
  inline static int Binomial(int t, double p) { SetBinomialPar(t,p); return BinomialRand(); }
  ...
  inline static int Geometric(double p) { SetGeometric(p); return GeometricRand(); }
  ...
  inline static int Poisson(double mean) { SetPoissonPar(mean); return PoissonRand(); }
  ...
  inline static double Exp(double lambda) { SetExpPar(lambda); return ExponentialRand(); }
  ...
  inline static double Gamma(double alpha, double beta) { SetGammaPar(alpha, beta); return GammaRand(); }
  ...
  inline static double Chi2(double n) { SetChi2Par(n); return Chi2Rand(); }
...
private: // the different implementations
  ...
  static const vector Cyl_imp(double rho, double hh, Random<vector>::Axis axis, std::true_type const &);
  static const vector Cyl_imp(double rho, double hh, Random<vector>::Axis axis, std::false_type const &);
...
public: // the wrappers around the implementations
  inline static const vector Direction() { return Dir_impl(has_setRThetaPhi<vector>()); }

  static const vector Sphere(double r);
  static const vector Sphere(double r, const vector& center);

  inline static const vector Cylinder(double rho, double hh, Random<vector>::Axis axis=Z) { return Cyl_imp(rho,hh,axis,has_setRhoPhiZ<vector>()); }
  static const vector Cylinder(double rho, double hh, const vector& center, Random<vector>::Axis axis=Z);

  inline static const vector Cuboid(double a, double b, double c) { return Cub_impl(a,b,c,has_setX<vector>()); }
...
{
  vector v;
  v.setRThetaPhi(1.,std::acos(1.-2.*UniformRand()),Math::TwoPi()*UniformRand());
  return std::move(v);
}
template<class vector>
...
{
  vector v;
  v.SetMagThetaPhi(1.,std::acos(1.-2.*UniformRand()),Math::TwoPi()*UniformRand());
  return std::move(v);
}

// Cylinder
template<class vector>
inline  const vector Random<vector>::Cyl_imp(double rho, double hh, Random<vector>::Axis axis, const std::true_type&)
{
  vector v;
  v.setRhoPhiZ(rho*UniformRand(),Math::TwoPi()*UniformRand(),hh*(2.*UniformRand()-1.));
  switch(axis) {
...
template<class vector>
inline const vector Random<vector>::Cyl_imp(double rho, double hh, Random<vector>::Axis axis, const std::false_type&)
{
  vector v;
  v.SetRhoPhiZ(rho*UniformRand(),Math::TwoPi()*UniformRand(),hh*(2.*UniformRand()-1.));
  switch(axis) {
...
// Cuboid
template<class vector>
inline const vector Random<vector>::Cub_impl(double a, double b, double c, const std::true_type&)
{
  vector v;
...
template<class vector>
inline const vector Random<vector>::Cub_impl(double a, double b, double c, const std::false_type&)
{
  vector v;
  v.SetX(a*(UniformRand()-0.5));
  v.SetY(b*(UniformRand()-0.5));
