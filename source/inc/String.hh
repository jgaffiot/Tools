...
constexpr hash_t HashCompileTime(const char* const str, hash_t last_value = detail_string::basis) {
  return *str ? HashCompileTime(str+1, (*str ^ last_value) * detail_string::prime) : last_value;
}
constexpr hash_t operator"" _hash(const char* const p, size_t) { return HashCompileTime(p); }
