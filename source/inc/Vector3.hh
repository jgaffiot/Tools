...
public:
  ...
  inline double operator()(std::size_t i) const { if (i<3) { return v[i]; } throw except("Vector3::operator(): index too high: ",i," > 2"); }
  ...
  inline void SetXYZ(double x, double y, double z) { v[0]=x; v[1]=y; v[2]=z; }
...
inline Vector3 operator - (const Vector3 &a, const Vector3 &b) { return Vector3(a.x() - b.x(), a.y() - b.y(), a.z() - b.z()); }
inline double  operator * (const Vector3 &a, const Vector3 &b) { return a.x()*b.x() + a.y()*b.y() + a.z()*b.z(); }
inline Vector3 operator * (const Vector3 &v, double d) { return Vector3(d*v.x(), d*v.y(), d*v.z()); }
inline Vector3 operator * (double d, const Vector3 &v) { return Vector3(d*v.x(), d*v.y(), d*v.z()); }
inline Vector3 operator / (const Vector3 &v, double d) { return Vector3(v.x()/d, v.y()/d, v.z()/d); }
