...
void display_bar_cxx98_notime(char* disp, const unsigned long whereami, const unsigned long tot, const bool kbs)
{
...
void display_bar_cxx98(char* disp, const unsigned long whereami, const unsigned long tot, const double init_time, const bool kbs)
{
  int progress=int(100.*double(whereami)/double(tot));
  char* p = disp;
  struct timeval tv;
  ...
  *p++='\0';
  ...
  if ( kbs) { std::sprintf(buffer,"  %.1f kB/s -> %.1f kB/s",speed,average); }
...
{
  now = PBC::now();
  if ( now - previous < delta_t ) { return; }
  ...
  *p++='\0';
  ...
  double speed = ( current_iter - previous_iter ) / double((now-previous).count()) * ClockPeriod;
