...
string DataBase::GetPath(const char* env_path) const
{
  ...
  if( not env ) {
    throw database_except(" ***Error DataBase::GetPath()2: env_path is not defined: ", env_path);
  }
...
void DataBase::ReadFile(istream& is, string prefix, DataBase::EOverwrite overwrite)
{
  ...
  while (getline(is,str).good()) { // get a line from the file
    ...
    if (str.empty() || str.front() == '#' || str.find_first_not_of(' ')==string::npos) { continue; }
    while (regComment.Match(str)) { regComment.Substitute(str,""); }
	...
	throw database_except(" ***Error DataBase::ReadFile2: Can not find table name in line: ",str);
      }
      const string tablename = regName.at(1);

      if ( TableMap.count(tablename)!=0 ) {
	throw database_except(" ***Error DataBase::ReadFile3: Attempt to redefine already defined table: ", tablename);
      }

//       TableMap[tablename] = make_shared<DataTable>(is,str);
//    Here using make_shared is not possible because the constructor of DataTable is private
    ...
    // always have an entry in both the numeric and string value dictionaries, possibly
    // zero(-length) if no number or no string value was supplied.
    ...
    if ( StringMap.count(regParse[1]) or NumMap.count(regParse[1]) ) {
      ...
      cerr << " *Warning DataBase::ReadFile2: " << (overwrite != kKeep ? "OVERRIDING" : "PRESERVING")
	   << " previous setting of " << key << endl;
      if (overwrite != kKeep) { continue; }
    } else {
      // name does not exist in hash, so add to vector of keys
      KeyList.push_back(key);
    }

    if (NbMatch>7) { StringMap[key] = regParse.at(7); } // case 'name numeric string [comment]'
...
void DataBase::ReadFile(const char* filename, const char* env_path, const char* prefix, DataBase::EOverwrite overwrite)
{
  string FileWithPath = filename;
  if (env_path) { FileWithPath = GetPath(env_path) + FileWithPath; }

  if (IsReadOnly) {
    throw database_except(" ***Error DataBase::ReadFile2: Read-only database, can not update: ", FileWithPath);
  }
  ...
  if (!ifs.is_open() || !ifs.good()) {
    throw database_except(" ***Error DataBase::ReadFile3: Could not open: ", FileWithPath);
  }

  if (prefix) { ReadFile(ifs, prefix, overwrite); }
  else { ReadFile(ifs, "", overwrite); }
  ifs.close();
}
...
void DataBase::WriteText(const char* filename, const char* env_path) const
{
  ...
  if (!ofs.is_open() || !ofs.good()) {
    throw database_except(" ***Error DataBase::WriteText: Could not open ", filename);
  }
...
double DataBase::GetNumValue(const string& key) const
{
  if ( NumMap.count(key)==0 ) {
    // if application code is using this method, it's just reading the db, not assigning to it.
    // This is always an error in DataBase if the key is not yet defined.
    throw database_except(" ***Error DataBase::GetNumValue: Attempt to retrieve undefined key ", key);
  }
...
const string& DataBase::GetStrValue(const string& key) const
{
  if ( StringMap.count(key)==0 ) {
    throw database_except(" ***Error DataBase::GetStrValue: Attempt to retrieve undefined key ", key);
  }
...
{
  if (IsReadOnly) {
    throw database_except(" ***Error DataBase::operator[]: Attempt to modify read-only base: ", key);
  }
  if ( NumMap.count(key)!=0 ) {
//     throw parambase_except(" ***Error DataBase::operator[]2: Attempt to create already existing key: ", key);
...
{
  if (IsReadOnly) {
    throw database_except(" ***Error DataBase::operator()1: Attempt to modify read-only base: ", key);
  }
  if ( StringMap.count(key)!=0 ) {
//     throw parambase_except(" ***Error DataBase::operator()2: Attempt to create already existing key: ", key);
    cerr << " *Warning DataBase::operator(): Overriding existing key: "<< key<<"\n";
...
const std::shared_ptr<DataTable> DataBase::GetTablePtr(const char* name) const
{
  if ( TableMap.count(string(name))==0 ) {
    throw database_except(" ***Error DataBase::GetTable:Attempt to access undefined table ", name);
  }
...
vector< double > DataTable::GetColumnAsNum(const string& colname) const
{
  if ( ColMap.count(colname)==0 ) {
    throw database_except(" ***Error DataTable::GetColumnAsNum: Unknown : ",colname);
  }
...
DataTable::DataTable(std::istream& is, std::string& line)
{
  ...
  // check if a comment is a the end of the line
  while (regSharp.Match(header)) { regComment.Substitute(header,""); } // remove if necessary
  ...
  while(getline(is,line).good()) {
    ...
    while (regSharp.Match(line)) { regComment.Substitute(line,""); } // remove if necessary

    // end of the table at the first blank line
    if (line.empty() || line.find_first_not_of(' ')==string::npos) { break; }

    // parse the line
    if (regSpace.Split(line).size()-1 != NbCol) {
      throw database_except(" ***Error DataTable::DataTable1: Bad line, expected ",
			    NbCol," columns, got ",regSpace.Nb()," values on row ",NbRow);
    }
...
#ifdef DEBUG
  cout << " --- Table successfully defined with "<<NbRow<<" rows and "<<ColNames.size()<<" columns found ---\n";
#endif
}
...
{
  for (auto& name : ColNames) { os << "\t" << name; }
  os << "\n";
...
void DataBase::WriteRoot(const char *filename) const
{
  ...
  if (!file.IsOpen()) {
    throw parambase_except(" ***Error DataBase::WriteRoot1: Could not open ", filename);
  }
  WriteRoot(&file);
  file.Close();
}
